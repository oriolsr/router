...
//#region src/history/html5.ts
	...
	*/
	function buildState(back, current, forward, replaced = false, computeScroll = false, layers) {
		return {
			back,
			current,
			forward,
	...
	function useHistoryStateNavigation(base) {
		...
		function replace(to, data) {
			const layers = data?.state;
			changeLocation(to, assign({}, history$1.state, buildState(historyState.value.back, to, historyState.value.forward, true, false, layers || historyState.value.state), data, { position: historyState.value.position }), true);
			currentLocation.value = to;
		}
		function push(to, data) {
			...
			if (__DEV__ && !history$1.state) warn("history.state seems to have been manually replaced without preserving the necessary values. Make sure to preserve existing history state if you are manually calling history.replaceState:\n\nhistory.replaceState(history.state, '', url)\n\nYou can find more information at https://router.vuejs.org/guide/migration/#Usage-of-history-state");
			changeLocation(currentState.current, currentState, true);
			const layers = data?.state;
			changeLocation(to, assign({}, buildState(currentLocation.value, to, null, false, false, layers), { position: currentState.position + 1 }, data), false);
			currentLocation.value = to;
		}
		return {
...
/**
	...
	*/
	function createMemoryHistory(base = "") {
		...
		if (__TEST__) routerHistory.changeURL = function(url, state = {}) {
			const from = this.location;
...
/**
	...
	*/
	function createWebHashHistory(base) {
		...
		if (__DEV__ && !base.endsWith("#/") && !base.endsWith("#")) warn(`A hash base must end with a "#":\n"${base}" should be "${base.replace(/#.*$/, "#")}".`);
...
/**
	...
	*/
	function createRouterError(type, params) {
		if (__DEV__ || !__BROWSER__) return assign(new Error(ErrorTypeMessages[type](params)), {
			type,
...
//#region src/matcher/pathTokenizer.ts
	...
	function tokenizePath(path) {
		...
		if (!path.startsWith("/")) throw new Error(__DEV__ ? `Route paths should start with a "/": "${path}" should be "/${path}".` : `Invalid path "${path}"`);
	...
	*/
	function createRouterMatcher(routes, globalOptions) {
		...
		function addRoute(record, parent, originalRecord) {
			...
			if (__DEV__) checkChildMissingNameWithEmptyPath(mainNormalizedRecord, parent);
			...
			for (const normalizedRecord of normalizedRecords) {
				...
				if (__DEV__ && normalizedRecord.path === "*") throw new Error("Catch all routes (\"*\") must now be defined using a param with a custom regexp.\nSee more at https://router.vuejs.org/guide/migration/#Removed-star-or-catch-all-routes.");
				...
				if (originalRecord) {
					...
					if (isRootAdd && record.name && !isAliasRecord(matcher)) {
						if (__DEV__) checkSameNameAsAncestor(record, parent);
		...
		function resolve(location$1, currentLocation) {
			...
			if ("name" in location$1 && location$1.name) {
				...
				if (__DEV__ && !path.startsWith("/")) warn(`The Matcher cannot resolve relative paths but received "${path}". Unless you directly called \`matcher.resolve("${path}")\`, this is probably a bug in vue-router. Please open an issue at https://github.com/vuejs/router/issues/new/choose.`);
	...
	*/
	function findInsertionIndex(matcher, matchers) {
		...
		if (insertionAncestor) {
			...
			if (__DEV__ && upper < 0) warn(`Finding ancestor route "${insertionAncestor.record.path}" failed for "${matcher.record.path}"`);
		}
		return upper;
	}
...
/**
	...
	* @internal
	*/
	const matchedRouteKey = Symbol(__DEV__ ? "router view location matched" : "");
	/**
	...
	* @internal
	*/
	const viewDepthKey = Symbol(__DEV__ ? "router view depth" : "");
	/**
	...
	* @internal
	*/
	const routerKey = Symbol(__DEV__ ? "router" : "");
	/**
	...
	* @internal
	*/
	const routeLocationKey = Symbol(__DEV__ ? "route location" : "");
	/**
	...
	* @internal
	*/
	const routerViewLocationKey = Symbol(__DEV__ ? "router view location" : "");
	/**
	...
	* @internal
	*/
	const routerLayerKey = Symbol(__DEV__ ? "router layer" : "");
	/**
	...
	* @internal
	*/
	const routerRoutesKey = Symbol(__DEV__ ? "router routes" : "");
...
//#region src/navigationGuards.ts
	...
	*/
	function onBeforeRouteLeave(leaveGuard) {
		if (__DEV__ && !(0, vue.getCurrentInstance)()) {
			warn("getCurrentInstance() returned null. onBeforeRouteLeave() must be called at the top of a setup function");
			return;
		}
		const activeRecord = (0, vue.inject)(matchedRouteKey, {}).value;
		if (!activeRecord) {
			__DEV__ && warn("No active route record was found when calling `onBeforeRouteLeave()`. Make sure you call this function inside a component child of <router-view>. Maybe you called it inside of App.vue?");
			return;
		}
	...
	*/
	function onBeforeRouteUpdate(updateGuard) {
		...
		const activeRecord = (0, vue.inject)(matchedRouteKey, {}).value;
		if (!activeRecord) {
			__DEV__ && warn("No active route record was found when calling `onBeforeRouteUpdate()`. Make sure you call this function inside a component child of <router-view>. Maybe you called it inside of App.vue?");
			return;
		}
	...
	function guardToPromiseFn(guard, to, from, record, name, runWithContext = (fn) => fn()) {
		...
		return () => new Promise((resolve, reject) => {
			...
			const guardReturn = runWithContext(() => guard.call(record && record.instances[name], to, from, __DEV__ ? canOnlyBeCalledOnce(next, to, from) : next));
	...
	*/
	function guardToPromiseFnWithLayers(guard, toRoutes, fromRoutes, runWithContext = (fn) => fn()) {
		return () => new Promise((resolve, reject) => {
			const next = (valid) => {
				if (valid === false) reject(createRouterError(ErrorTypes.NAVIGATION_ABORTED, {
			...
			const guardReturn = runWithContext(() => guard.call(void 0, toRoutes, fromRoutes, __DEV__ ? canOnlyBeCalledOnceForLayers(next, toRoutes, fromRoutes) : next));
			let guardCall = Promise.resolve(guardReturn);
			if (guard.length < 3) guardCall = guardCall.then(next);
			if (__DEV__ && guard.length > 2) {
				toRoutes[toRoutes.length - 1] || toRoutes[0];
				fromRoutes[fromRoutes.length - 1] || fromRoutes[0];
				const message = `The "next" callback was never called inside of ${guard.name ? "\"" + guard.name + "\"" : ""}:\n${guard.toString()}\n. If you are returning a value instead of calling "next", make sure to remove the "next" parameter from your function.`;
				if (typeof guardReturn === "object" && "then" in guardReturn) guardCall = guardCall.then((resolvedValue) => {
					if (!next._called) {
						warn(message);
	...
	function canOnlyBeCalledOnceForLayers(next, toRoutes, fromRoutes) {
		let called = 0;
	...
	function extractComponentsGuards(matched, guardType, to, from, runWithContext = (fn) => fn()) {
		...
		for (const record of matched) {
			if (__DEV__ && !record.components && (!record.children || record.children.length === 0) && record.path !== "") warn(`Record with path "${record.path}" is either missing a "component(s)" or "children" property.`);
...
/**
	...
	*/
	function useLink(props) {
		...
		function navigate(e = {}) {
			if (guardEvent(e)) {
				const replace = (0, vue.unref)(props.replace);
				const addLayer = (0, vue.unref)(props.addLayer);
				const removeLayer = (0, vue.unref)(props.removeLayer);
				const to = (0, vue.unref)(props.to);
				const currentLayer = (0, vue.inject)(routerLayerKey, 0);
				const layerIndex = typeof currentLayer === "number" ? currentLayer : (0, vue.unref)(currentLayer);
				let p;
				if (addLayer) p = router[replace ? "replaceAddLayer" : "pushAddLayer"](to).catch(noop);
				else if (removeLayer) p = router[replace ? "replaceRemoveLayer" : "pushRemoveLayer"]().catch(noop);
				else p = router[replace ? "replaceLayer" : "pushLayer"](layerIndex, to).catch(noop);
...
/**
	...
	*/
	function callLifecycleHook(instance, hookName) {
		try {
			const optionsHook = instance?.$options?.[hookName];
			if (typeof optionsHook === "function") optionsHook.call(instance);
			else if (Array.isArray(optionsHook)) optionsHook.forEach((hook) => {
				if (typeof hook === "function") hook.call(instance);
			});
			const compositionHooks = instance?.[hookName];
			if (Array.isArray(compositionHooks)) compositionHooks.forEach((hook) => {
				if (typeof hook === "function") hook();
			});
		} catch (error) {
			if (__DEV__) warn(`Error calling ${hookName} hook: ${error}`);
		}
	}
	/**
	* Trigger lifecycle hooks on all matched route instances for a specific layer
	*/
	function callHooksOnLayer(routes, layer, hookName) {
		if (layer >= routes.length || !routes[layer]) return;
		routes[layer].matched.forEach((record) => {
			Object.values(record.instances).forEach((instance) => {
				if (instance) callLifecycleHook(instance, hookName);
			});
		});
	}
	const RouterViewImpl = /* @__PURE__ */ (0, vue.defineComponent)({
		...
		setup(props, { attrs, slots }) {
			...
			const injectedLayer = (0, vue.inject)(routerLayerKey, 0);
			const injectedRoutes = (0, vue.inject)(routerRoutesKey);
			const layerIndex = (0, vue.computed)(() => {
				const baseLayer = typeof injectedLayer === "number" ? injectedLayer : (0, vue.unref)(injectedLayer);
				return props.nextLayer ? baseLayer + 1 : baseLayer;
			});
			const routeToDisplay = (0, vue.computed)(() => {
				if (props.route) return props.route;
				const layer = layerIndex.value;
				if (injectedRoutes) {
					const routes = injectedRoutes.value;
					if (layer < routes.length && routes[layer] && routes[layer].matched && routes[layer].matched.length > 0) return routes[layer];
					return START_LOCATION_NORMALIZED;
				}
				if (layer === 0) {
					const route = injectedRoute.value;
					if (route && route.matched && route.matched.length > 0) return route;
					return START_LOCATION_NORMALIZED;
				}
				return START_LOCATION_NORMALIZED;
			});
			...
			(0, vue.provide)(routerLayerKey, layerIndex);
			const viewRef = (0, vue.ref)();
			if (injectedLayer === 0 && injectedRoutes) (0, vue.watch)(() => injectedRoutes.value.length, (newLayerCount, oldLayerCount) => {
				if (oldLayerCount === 1 && newLayerCount === 2) callHooksOnLayer(injectedRoutes.value, 0, "deactivated");
				else if (oldLayerCount === 2 && newLayerCount === 1) callHooksOnLayer(injectedRoutes.value, 0, "activated");
			}, { flush: "pre" });
			(0, vue.watch)(() => [
				viewRef.value,
			...
			return () => {
				...
				if (!route || typeof route !== "object" || !("matched" in route)) return normalizeSlot(slots.default, {
					Component: null,
				...
				if (ViewComponent && typeof ViewComponent === "object" && "then" in ViewComponent) return normalizeSlot(slots.default, {
					Component: null,
					route
				});
				if (!ViewComponent || !matchedRoute) return normalizeSlot(slots.default, {
...
/**
	...
	*/
	function createRouter(options) {
		...
		if (__DEV__ && !routerHistory) throw new Error("Provide the \"history\" option when calling \"createRouter()\": https://router.vuejs.org/api/interfaces/RouterOptions.html#history");
		const beforeGuards = useCallbacks();
		const beforeResolveGuards = useCallbacks();
		const afterGuards = useCallbacks();
		const currentRoutes = (0, vue.shallowRef)([START_LOCATION_NORMALIZED]);
		const currentRoute = (0, vue.computed)(() => {
			return currentRoutes.value[currentRoutes.value.length - 1];
		});
		let pendingLocations = [START_LOCATION_NORMALIZED];
		...
		function addRoute(parentOrRoute, route) {
			...
			if (isRouteName(parentOrRoute)) {
				...
				if (__DEV__ && !parent) warn(`Parent route "${String(parentOrRoute)}" not found when adding child route`, route);
				record = route;
		...
		function removeRoute(name) {
			...
			else if (__DEV__) warn(`Cannot remove non-existent route "${String(name)}"`);
		}
		function getRoutes() {
		...
		function resolve(rawLocation, currentLocation) {
			...
			if (rawLocation.path != null) {
				if (__DEV__ && "params" in rawLocation && !("name" in rawLocation) && Object.keys(rawLocation.params).length) warn(`Path "${rawLocation.path}" was passed with params but they will be ignored. Use a named route alongside params instead.`);
			...
			if (__DEV__ && hash && !hash.startsWith("#")) warn(`A \`hash\` should always start with the character "#". Replace "${hash}" with "#${hash}".`);
		...
		function checkCanceledNavigation(to, from) {
			...
			if (lastPending !== to) {
				if (__DEV__) console.warn("[Router] Navigation cancelled - pending location mismatch", {
		...
		function pushAddLayer(to) {
			return navigateAllLayers([...currentRoutes.value.map((r) => r.fullPath), resolve(to).fullPath], true);
		}
		function replaceAddLayer(to) {
			return navigateAllLayers([...currentRoutes.value.map((r) => r.fullPath), resolve(to).fullPath], false);
		}
		function pushRemoveLayer() {
			if (currentRoutes.value.length <= 1) return Promise.resolve();
			return navigateAllLayers(currentRoutes.value.slice(0, -1).map((r) => r.fullPath), true);
		}
		function replaceRemoveLayer() {
			if (currentRoutes.value.length <= 1) return Promise.resolve();
			return navigateAllLayers(currentRoutes.value.slice(0, -1).map((r) => r.fullPath), false);
		}
		function pushLayer(layer, to) {
			return navigateAllLayers([
				...currentRoutes.value.slice(0, layer).map((r) => r.fullPath),
				resolve(to).fullPath,
				...currentRoutes.value.slice(layer + 1).map((r) => r.fullPath)
			], true);
		}
		function replaceLayer(layer, to) {
			replaceLayerDirect(layer, to);
			return Promise.resolve();
		}
		function replaceLayerDirect(layer, to) {
			...
			const limitedLocations = [
				...
				return resolve(loc, index < currentRoutes.value.length ? currentRoutes.value[index] : currentRoute.value);
			}).slice(-2);
			if (limitedLocations.length === 2) {
				const [first, second] = limitedLocations;
				if (first.fullPath === second.fullPath) limitedLocations.splice(0, 1);
			}
			if (limitedLocations.some((loc) => {
				return loc.matched.some((record) => record.components && Object.values(record.components).some((comp) => typeof comp === "function"));
			})) Promise.all(limitedLocations.map((loc) => loadRouteLocation(loc))).then((loadedLocations) => {
				updateRoutesDirectly(loadedLocations);
			}).catch(() => {
				updateRoutesDirectly(limitedLocations.map((loc) => loc));
			});
			else updateRoutesDirectly(limitedLocations.map((loc) => {
				return currentRoutes.value.find((r) => isSameRouteLocation(stringifyQuery$1, r, loc)) || loc;
			}));
			function updateRoutesDirectly(loadedLocations) {
				pendingLocations = loadedLocations.map((loc) => loc);
				...
				const isFirstNavigation = fromLocations.length === 0 || fromLocations[0] === START_LOCATION_NORMALIZED;
				const state = !isBrowser ? {} : history.state;
				routerHistory.replace(lastLocation.fullPath, assign({
					scroll: isFirstNavigation && state && state.scroll,
					state: layers
				}, {}));
				currentRoutes.value = loadedLocations;
				handleScroll(lastLocation, fromLocations[fromLocations.length - 1] || START_LOCATION_NORMALIZED, true, isFirstNavigation);
			}
		}
		function pushAllLayers(locations) {
			return navigateAllLayers(locations.map((loc) => resolve(loc).fullPath), true);
		}
		function replaceAllLayers(locations) {
			return navigateAllLayers(locations.map((loc) => resolve(loc).fullPath), false);
		}
		function navigateAllLayers(locations, push$1) {
			if (locations.length === 0) return Promise.resolve();
			const resolvedLocations = locations.slice(-2).map((loc, index) => {
				return resolve(loc, index < currentRoutes.value.length ? currentRoutes.value[index] : currentRoute.value);
			});
			if (resolvedLocations.length === 2) {
				const [first, second] = resolvedLocations;
				if (first.fullPath === second.fullPath) resolvedLocations.splice(0, 1);
			}
			const lastLocation = resolvedLocations[resolvedLocations.length - 1];
			pendingLocations = resolvedLocations;
			const locationOptions = push$1 ? {} : { replace: true };
			return pushWithRedirect(assign(locationAsObject(lastLocation), locationOptions));
		}
		...
		function pushWithRedirect(to, redirectedFrom) {
			...
			const isRedirect = redirectedFrom !== void 0;
			const lastPending = hadMultipleLayers ? pendingLocations[pendingLocations.length - 1] : null;
			const lastPendingMatchesTarget = lastPending && lastPending.fullPath === targetLocation.fullPath;
			if (hadMultipleLayers && (isRedirect || lastPendingMatchesTarget)) {
				if (isRedirect) pendingLocations[pendingLocations.length - 1] = targetLocation;
				else if (lastPendingMatchesTarget) pendingLocations[pendingLocations.length - 1] = targetLocation;
			} else pendingLocations = [targetLocation];
			...
			return (failure ? Promise.resolve(failure) : navigate(toLocation, from)).catch((error) => isNavigationFailure(error) ? isNavigationFailure(error, ErrorTypes.NAVIGATION_GUARD_REDIRECT) ? error : markAsReady(error) : triggerError(error, toLocation, from)).then((failure$1) => {
				if (failure$1) {
					if (isNavigationFailure(failure$1, ErrorTypes.NAVIGATION_GUARD_REDIRECT)) {
						if (__DEV__ && isSameRouteLocation(stringifyQuery$1, resolve(failure$1.to), toLocation) && redirectedFrom && (redirectedFrom._count = redirectedFrom._count ? redirectedFrom._count + 1 : 1) > 30) {
				...
				} else failure$1 = finalizeNavigation(pendingLocations.length > 1 ? pendingLocations.map((loc, index) => {
					if (index === pendingLocations.length - 1) return toLocation;
					return loc;
				}) : [toLocation], currentRoutes.value.length > 1 ? currentRoutes.value : [from], true, replace$1, data);
		...
		*/
		...
		function navigate(to, from) {
			...
			return runGuardQueue(guards).then(() => {
				guards = [];
				let toRoutes = pendingLocations.length > 1 ? pendingLocations.slice(-2).map((loc) => {
					if ("matched" in loc && loc.matched) return loc;
					return resolve(loc.fullPath || loc.path || "");
				}) : [to];
				if (toRoutes.length === 2) {
					const [first, second] = toRoutes;
					if (first.fullPath === second.fullPath) toRoutes = [second];
				}
				const fromRoutes = currentRoutes.value.slice(-2);
				const beforeGuardsList = beforeGuards.list();
				for (const guard of beforeGuardsList) guards.push(guardToPromiseFnWithLayers(guard, toRoutes, fromRoutes, runWithContext));
				guards.push(canceledNavigationCheck);
				...
				if (pendingLocations.length > 1) {
					const loadPromises = pendingLocations.map((loc) => {
						if ("matched" in loc && loc.matched && loc.matched.length > 0) {
							if (!loc.matched.some((record) => record.components && Object.values(record.components).some((comp) => typeof comp === "function" && !("displayName" in comp)))) return Promise.resolve(loc);
						}
						return loadRouteLocation(loc);
					});
					return Promise.all(loadPromises).then((loadedLocations) => {
						pendingLocations = loadedLocations;
		...
		*/
		function finalizeNavigation(toLocations, fromLocations, isPush, replace$1, data) {
			...
			if (isPush) {
				...
				if (replace$1 || isFirstNavigation) routerHistory.replace(toLocation.fullPath, assign({
					scroll: isFirstNavigation && state && state.scroll,
					state: layers
				}, data));
				else routerHistory.push(toLocation.fullPath, assign({ state: layers }, data));
			}
			const currentLastRoute = currentRoutes.value[currentRoutes.value.length - 1];
			if (currentLastRoute && isSameRouteLocation(stringifyQuery$1, currentLastRoute, toLocation) && toLocations.length === 1 && currentRoutes.value.length === 1) {} else {
				let newCurrentRoutes;
				if (toLocations.length === 1) newCurrentRoutes = toLocations;
				else if (pendingLocations.length > 1 && toLocations.length === pendingLocations.length) newCurrentRoutes = toLocations.slice(-2);
				else if (pendingLocations.length > 1 && toLocations.length > 1) newCurrentRoutes = pendingLocations.slice(-2).map((loc, index) => {
					const toLocationsIndex = toLocations.length - 2 + index;
					if (toLocationsIndex >= 0 && toLocationsIndex < toLocations.length) return toLocations[toLocationsIndex];
					return resolve(loc.fullPath);
				});
				else newCurrentRoutes = toLocations.slice(-1);
		...
		function setupListeners() {
			...
			removeHistoryListener = routerHistory.listen((to, _from, info) => {
				if (!router.listening) {
					if (__DEV__) console.warn("[Router] History listener called but router.listening is false");
					return;
				}
				...
				if (info.layers && info.layers.length > 0) {
					const resolvedLayers = info.layers.map((layerPath, index) => {
						return resolve(layerPath, index < currentRoutes.value.length ? currentRoutes.value[index] : currentRoute.value);
					});
					if (resolvedLayers.length === 1 && resolvedLayers[0].fullPath === toLocation.fullPath) pendingLocations = [toLocation];
					else pendingLocations = resolvedLayers;
				} else pendingLocations = [toLocation];
				const from = currentRoute.value;
				if (isBrowser) saveScrollPosition(getScrollKey(from.fullPath, info.delta), computeScrollPosition());
				navigate(toLocation, from).then((failure) => {}).catch((error) => {
					...
					const finalFailure = failure || finalizeNavigation([toLocation], [from], false);
					if (finalFailure) {
						if (info.delta && !isNavigationFailure(finalFailure, ErrorTypes.NAVIGATION_CANCELLED)) routerHistory.go(-info.delta, false);
						else if (info.type === NavigationType.pop && isNavigationFailure(finalFailure, ErrorTypes.NAVIGATION_ABORTED | ErrorTypes.NAVIGATION_DUPLICATED)) routerHistory.go(-1, false);
					}
					triggerAfterEach(toLocation, from, finalFailure);
				}).catch(noop);
			});
		}
		...
		*/
		function triggerError(error, to, from) {
			...
			if (list.length) list.forEach((handler) => handler(error, to, from));
				...
				if (__DEV__) warn("uncaught error during route navigation:");
				console.error(error);
			}
