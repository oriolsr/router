 ...
 * @license MIT
 */
import { $ as isArray, A as scrollToPosition, C as isNavigationFailure, D as getSavedScrollPosition, E as computeScrollPosition, G as decode, H as resolveRelativePath, I as NEW_stringifyURL, J as encodePath, L as START_LOCATION_NORMALIZED, M as NavigationType, O as getScrollKey, Q as identityFn, R as isSameRouteLocation, S as createRouterError, V as parseURL, Y as warn$1, Z as assign, _ as normalizeQuery, b as ErrorTypes, d as routeLocationKey, f as routerKey, h as routerViewLocationKey, i as guardToPromiseFn, k as saveScrollPosition, l as useCallbacks, n as extractChangingRecords, nt as isBrowser, q as encodeParam, r as extractComponentsGuards, t as addDevtools, tt as noop, v as parseQuery, y as stringifyQuery } from "../devtools-BBdyC1XS.mjs";
	...
	*/
	...
	const router = {
		...
		install(app) {
			...
			if ((__DEV__ || __FEATURE_PROD_DEVTOOLS__) && isBrowser) addDevtools(app, router, resolver);
		}
	};
	function runGuardQueue(guards) {
		return guards.reduce((promise, guard) => promise.then(() => runWithContext(guard)), Promise.resolve());
	}
	if (__DEV__) router._hmrReplaceResolver = (newResolver) => {
...
var MatcherPatternPathDynamic = class {
	...
	*/
	...
	match(path) {
		...
		if (__DEV__ && Object.keys(params).length !== Object.keys(this.params).length) warn$1(`Regexp matched ${match.length} params, but ${i} params are defined. Found when matching "${path}" against ${String(this.re)}`);
		return params;
	}
	build(params) {
		let paramIndex = 0;
